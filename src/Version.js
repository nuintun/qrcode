...
export default function Version(versionNumber, alignmentPatternCenters, ecBlocks) {
  var context = this;

  context.versionNumber = versionNumber;
  context.alignmentPatternCenters = alignmentPatternCenters;
  context.ecBlocks = ecBlocks;

  ecBlocks = [].slice.call(arguments, 2);
  ...
  for (var i = 0, length = ecbArray.length; i < length; i++) {
    ecBlock = ecbArray[i];

    total += ecBlock.getCount() * (ecBlock.getDataCodewords() + ecCodewords);
  }

  context.totalCodewords = total;
}
...
Version.prototype = {
  ...
  buildFunctionPattern: function() {
    ...
    // Alignment patterns
    var alignmentPatternCenters = this.alignmentPatternCenters
...
Version.getProvisionalVersionForDimension = getProvisionalVersionForDimension;
Version.getVersionForNumber = getVersionForNumber;
Version.decodeVersionInformation = decodeVersionInformation;
