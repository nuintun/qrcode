...
export default class MatrixUtil {
  private static POSITION_DETECTION_PATTERN: Int32Array[] = Array.from([
  ...
  private static POSITION_ADJUSTMENT_PATTERN: Int32Array[] = Array.from([
  ...
  private static POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE: Int32Array[] = Array.from([
  ...
  private static TYPE_INFO_COORDINATES: Int32Array[] = Array.from([
    Int32Array.from([8, 0]),
    Int32Array.from([8, 1]),
    Int32Array.from([8, 2]),
    Int32Array.from([8, 3]),
  ...
  private static VERSION_INFO_POLY: number = 0x1f25; // 1 1111 0010 0101

  // From Appendix C in JISX0510:2004 (p.65).
  private static TYPE_INFO_POLY: number = 0x537;
  private static TYPE_INFO_MASK_PATTERN: number = 0x5412;
  ...
  public static embedBasicPatterns(version: Version, matrix: ByteMatrix): void {
  ...
  public static embedTypeInfo(ecLevel: ErrorCorrectionLevel, maskPattern: number, matrix: ByteMatrix): void {
    const typeInfoBits: BitArray = new BitArray();

    MatrixUtil.makeTypeInfoBits(ecLevel, maskPattern, typeInfoBits);

    for (let i: number = 0, size: number = typeInfoBits.getSize(); i < size; ++i) {
      ...
      if (i < 8) {
        // Right top corner.
        const x2: number = matrix.getWidth() - i - 1;
        const y2: number = 8;
        ...
        // Left bottom corner.
        const x2: number = 8;
        const y2: number = matrix.getHeight() - 7 + (i - 8);
  ...
  public static maybeEmbedVersionInfo(version: Version, matrix: ByteMatrix): void {
    if (version.getVersionNumber() < 7) {
      // Version info is necessary if version >= 7.
      return; // Don't need version info.
    }

    const versionInfoBits: BitArray = new BitArray();

    MatrixUtil.makeVersionInfoBits(version, versionInfoBits);

    let bitIndex: number = 6 * 3 - 1; // It will decrease from 17 to 0.
  ...
  public static embedDataBits(dataBits: BitArray, maskPattern: number, matrix: ByteMatrix): void {
    let bitIndex: number = 0;
    let direction: number = -1;
    // Start from the right bottom cell.
    let x: number = matrix.getWidth() - 1;
    let y: number = matrix.getHeight() - 1;
  ...
  // - findMSBSet(1) => 1
  // - findMSBSet(255) => 8
  public static findMSBSet(value: number): number {
  ...
  public static calculateBCHCode(value: number, poly: number): number {
    if (poly === 0) {
    ...
    const msbSetInPoly: number = MatrixUtil.findMSBSet(poly);
  ...
  public static makeTypeInfoBits(ecLevel: ErrorCorrectionLevel, maskPattern: number, bits: BitArray): void {
    if (!QRCode.isValidMaskPattern(maskPattern)) {
      throw new WriterException('Invalid mask pattern');
    }

    const typeInfo: number = (ecLevel.getBits() << 3) | maskPattern;

    bits.appendBits(typeInfo, 5);

    const bchCode: number = MatrixUtil.calculateBCHCode(typeInfo, MatrixUtil.TYPE_INFO_POLY);

    bits.appendBits(bchCode, 10);

    const maskBits: BitArray = new BitArray();
  ...
  public static makeVersionInfoBits(version: Version, bits: BitArray): void {
    bits.appendBits(version.getVersionNumber(), 6);

    const bchCode: number = MatrixUtil.calculateBCHCode(version.getVersionNumber(), MatrixUtil.VERSION_INFO_POLY);
  ...
  private static isEmpty(value: number): boolean {
    return value === 255; // -1
  }
  ...
  private static embedTimingPatterns(matrix: ByteMatrix): void {
    ...
    for (let i: number = 8; i < matrix.getWidth() - 8; ++i) {
      const bit: number = (i + 1) % 2;
  ...
  private static embedDarkDotAtLeftBottomCorner(matrix: ByteMatrix): void {
  ...
  private static embedHorizontalSeparationPattern(xStart: number, yStart: number, matrix: ByteMatrix): void {
  ...
  private static embedVerticalSeparationPattern(xStart: number, yStart: number, matrix: ByteMatrix): void {
  ...
  private static embedPositionAdjustmentPattern(xStart: number, yStart: number, matrix: ByteMatrix): void {
  ...
  private static embedPositionDetectionPattern(xStart: number, yStart: number, matrix: ByteMatrix): void {
    for (let y: number = 0; y < 7; ++y) {
  ...
  private static embedPositionDetectionPatternsAndSeparators(matrix: ByteMatrix): void {
    // Embed three big squares at corners.
    const pdpWidth: number = MatrixUtil.POSITION_DETECTION_PATTERN[0].length;
  ...
  private static maybeEmbedPositionAdjustmentPatterns(version: Version, matrix: ByteMatrix): void {
    ...
    const index: number = version.getVersionNumber() - 1;
    const coordinates: Int32Array = MatrixUtil.POSITION_ADJUSTMENT_PATTERN_COORDINATE_TABLE[index];

    for (let i: number = 0, length = coordinates.length; i !== length; i++) {
      const y: number = coordinates[i];

      if (y >= 0) {
        for (let j: number = 0; j !== length; j++) {
          const x: number = coordinates[j];
